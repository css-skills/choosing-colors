...
choropleth <- unemp_sf %>%
  ...
  labs(fill = "2010") +
  theme_void(base_size = rcfss::base_size - 2)
...
state_inc %>%
  ...
  theme_void(base_size = rcfss::base_size - 6)
```
...
```{r}
include_graphics(path = here("images", "seq-div.png"))
```
...
```{r}
include_graphics(path = here("images", "class-scale.png"))
```
...
```{r}
include_graphics(path = here("images", "class-continuous.png"))
```
...
ggplot(data = unemp_oct21, mapping = aes(fill = unemp)) +
  ...
  scale_fill_viridis_c(direction = -1, labels = percent_format(accuracy = 1)) +
  labs(title = "Unemployment rate", subtitle = "October 2020", fill = NULL) +
  theme_void(base_size = rcfss::base_size - 4) +
  theme(
    legend.position = "bottom",
